...
)
{
    ...
    if (iftValidVoxel(img, v)) {
        u_idx = iftGetVoxelIndex(resliced_img, u);

        // Verify if adjacent voxels are outside image domain (y)
        if (q_array[1].y >= img->ysize) {
            iq_24 = (
                (p_.y - (float)q_array[3].y)
                * img->val[iftGetVoxelIndex(img, q_array[1])]
            );
        }
        else {
            iq_24 = (
                ...
                * img->val[iftGetVoxelIndex(img, q_array[3])]
            );
        }
        if (q_array[5].y >= img->ysize) {
            iq_68 = (
                (p_.y - (float)q_array[7].y)
                * img->val[iftGetVoxelIndex(img, q_array[5])]
            );
        }
        else {
            iq_68 = (
                ...
                * img->val[iftGetVoxelIndex(img, q_array[7])]
            );
        }
        if (q_array[0].y >= img->ysize) {
            iq_13 = (
                (p_.y - (float)q_array[2].y)
                * img->val[iftGetVoxelIndex(img, q_array[0])]
            );
        }
        else {
            iq_13 = (
                ...
                * img->val[iftGetVoxelIndex(img, q_array[2])]
            );
        }
        if (q_array[4].y >= img->ysize) {
            iq_57 = (
                (p_.y - (float)q_array[6].y)
                * img->val[iftGetVoxelIndex(img, q_array[4])]
            );
        }
        else {
            iq_57 = (
                ...
                * img->val[iftGetVoxelIndex(img, q_array[6])]
            );
        }
        ...
        resliced_img->val[u_idx] = intensity;
    }
}