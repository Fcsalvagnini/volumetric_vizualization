    ...
    for (u.x = 0; u.x < renderedImg->xsize; u.x++) {
        for (u.y = 0; u.y < renderedImg->ysize; u.y++) {
            ...
            if (VerifyIntersection(
                ...
                // Finds ISO Surface
                YCbCr = GenerateTransparentColor(
                ...
                if (YCbCr != NULL) {
                    ...
                    renderedImg->val[uIdx] = YCbCr[0].val[0];
                    renderedImg->Cb[uIdx] = YCbCr[0].val[1];
                    renderedImg->Cr[uIdx] = YCbCr[0].val[2];
                    free(YCbCr);
                }
...
iftImage *Rendering(
    ...
    */
    ...
    // Computes Phong's for each visible object surface (With transparency)
