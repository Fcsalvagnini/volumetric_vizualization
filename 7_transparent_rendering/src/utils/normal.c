...
void ComputesNormalScene(
    ...
    printf("[INFO] Computing normal based on scene\n");
    int p = 0, q = 0, nIdx;
    iftVoxel u, v;
...
{
    printf("[INFO] sEDT: Computing normal based on object\n");
    int p = 0;
    iftAdjRel *A = iftSpheric(sqrtf(3));
    iftGQueue *Q = NULL;
    // iftVector grad, tmpGrad;
    // float intensityGrad, vectorMagnitude;
    // Cost Image
    iftImage *cost = iftCreateImageFromImage(maskImg);
    iftImage *root = iftCreateImageFromImage(maskImg);
    iftSetImage(cost, IFT_INFINITY_INT);
    iftSetImage(root, IFT_INFINITY_INT);

    Q = iftCreateGQueue(
        powf(2, 16) - 1.0, // n buckets
        maskImg->n, // n elements
        cost->val // Elements values
    );

    iftSet *auxS = iftSetCopy(S);
    while (S != NULL) {
        p = iftRemoveSet(&S);
        cost->val[p] = 0;
        root->val[p] = p;
    ...
    while (!iftEmptyGQueue(Q)) {
        p = iftRemoveGQueue(Q);
        u = iftGetVoxelCoord(maskImg, p);

        if (sqrt(cost->val[p]) <= pDistance) {
            rootU = iftGetVoxelCoord(maskImg, root->val[p]);

            for (size_t adjIdx = 1; adjIdx < A->n; adjIdx++) {
                v = iftGetAdjacentVoxel(A, u, adjIdx);
                if (iftValidVoxel(maskImg, v)) {
                    q = iftGetVoxelIndex(maskImg, v);
                    if (cost->val[q] > cost->val[p]) {
                        tmp = pow(v.x - rootU.x, 2)
                                + pow(v.y - rootU.y, 2)
                                + pow(v.z - rootU.z, 2);
                        if (tmp < cost->val[q]) {
    ...
    // Corrects orientation outside object borders
    for (size_t idx = 0; idx < cost->n; idx++) {
        if (cost->val[idx] == IFT_INFINITY_INT) {
            cost->val[idx] = 0;
        }
        else {
            if (maskImg->val[idx] != label) {
                cost->val[idx] = -cost->val[idx];
            }
        }
    }

    // Computes gradient
    ComputesObjectGradient(cost, graphicalContext, auxS);
...
)
{
    ...
    iftAdjRel *A = iftSpheric(3.0);
    iftVector grad, tmpGrad;
    float costGrad, vectorMagnitude;
    while (auxS != NULL) {
        p = iftRemoveSet(&auxS);
        ...
        for (size_t adjIdx = 1; adjIdx < A->n; adjIdx++) {
            ...
            if (iftValidVoxel(cost, v)) {
                ...
                costGrad = cost->val[q] - cost->val[p];
                tmpGrad = (iftVector) {
                    .x = v.x - u.x,
                    .y = v.y - u.y,
                    .z = v.z - u.z
                ...
                vectorMagnitude = iftVectorMagnitude(tmpGrad);
                tmpGrad = (iftVector) {
                    .x = costGrad * (tmpGrad.x / vectorMagnitude),
                    .y = costGrad * (tmpGrad.y / vectorMagnitude),
                    .z = costGrad * (tmpGrad.z / vectorMagnitude)
                };
                grad = (iftVector) {
                    .x = grad.x + tmpGrad.x,
                    .y = grad.y + tmpGrad.y,
        ...
        // Corrects vector orientation and unit vector
        vectorMagnitude = iftVectorMagnitude(grad);
        grad = (iftVector) {
            .x = -(grad.x / vectorMagnitude),
            .y = -(grad.y / vectorMagnitude),
